...
class User(AbstractUser):
    username = models.CharField('Логин', max_length=150, unique=True, help_text='Обязательное поле. Только латиницв, цифры и тире.', validators=[LoginValidator()],
                                error_messages={"unique": _("Пользователь с таким именем существует."), })
    first_name = models.CharField(_("first_name"), max_length=150, validators=[NameValidator()])
    last_name = models.CharField(_("last_nmae"), max_length=150, validators=[NameValidator()])
    patronymic = models.CharField('Отвество', max_length=150, blank=True, validators=[NameValidator()])
...
class Order(AbstractModel):
    ...
    count_products.fget.short_description = "Количество"
